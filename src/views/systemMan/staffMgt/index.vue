  ...
  <div class="staffMgt">
    <div class="main-content">
      <div class="bread-contain">
        ...
        <div class="bread-contain-right" v-show="staffMgtShow">
          <el-button class="el-icon-plus" type="primary" @click="appAdd('add')" size="mini">添加</el-button>
        </div>
        <div class="bread-contain-right" v-show="staffMgtAddVisible">
          <el-button size="mini" type="primary" @click="submitForm('staffMgtAddRuleForm')">保存</el-button>
          <el-button icon="el-icon-caret-left" size="mini" @click="handleClose">返回</el-button>
        </div>
      </div>
      <div class="company-content" v-show='staffMgtShow'>
        ...
        <div class="company-right kl-table">
          ...
          <el-table stripe border :data="appServerData.list" v-if="tableShow" :max-height="maxHeight">
            ...
            <el-table-column prop="matrNo" label="姓名" min-width="100">
            </el-table-column>
            <el-table-column prop="matrNo" label="帐号" min-width="100">
            </el-table-column>
            <el-table-column prop="matrNo" label="姓别" min-width="100">
            </el-table-column>
      ...
      <div v-show="staffMgtAddVisible">
        <staffMgtAdd ref="staffMgtAdd"></staffMgtAdd>
      </div>
    </div>
  </div>
</template>
...
<script>
import staffMgtAdd from "./staffMgtAdd"
export default {
  name: "staffMgt",
  components: {
    staffMgtAdd
    ...
    return {
      ...
      crumbsData: {  //面包屑
        titleList: [
          ...
          { title: '职员管理', method: () => { window.histroy.back() } }
        ],
  ...
  methods: {
    ...
    // 添加按钮点击事件
    appAdd(val) {
      this.staffMgtShow = false;
      this.staffMgtAddVisible = true;
      if (val === 'add') {
        this.$refs.staffMgtAdd.editData({
          matrNo: "",
          matrName: "",
          matrMode: "",
          matrNum: "",
        ...
        this.common.chargeObjectEqual(this, this.formData, 'set', 'staffMgtAdd');
      } else {
        ...
        this.$api.fetch({
          ...
          this.$refs.staffMgtAdd.editData(res);
          this.common.chargeObjectEqual(this, res, 'set', 'staffMgtAdd');
        })
      }
    ...
    selectTSubSystem() {
      ...
      //   _this.common.changeTable(_this,'.staffMgt .kl-table',[]);
      // })

    },
    closeDialog() {
      this.staffMgtShow = true;
      this.staffMgtAddVisible = false;
      this.crumbsData.titleList.pop();
    },
    // 返回
    handleClose() {

      this.$refs.staffMgtAdd.handleClose();
    },
    //保存按钮
    submitForm(formName) {
      let type = this.crumbsData.titleList[2].title;
      this.$refs.staffMgtAdd.submitForm(formName, type);
    },
