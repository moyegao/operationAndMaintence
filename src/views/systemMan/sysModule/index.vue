  ...
  <div class="sysModule">
    <div class="main-content">
      <div class="bread-contain">
        ...
        <div class="bread-contain-right" v-show="sysModuleShow">
          <el-button class="el-icon-plus" type="primary" @click="appAdd('add')" size="mini">添加</el-button>
        </div>
        <div class="bread-contain-right" v-show="sysModuleAddVisible">
          <el-button size="mini" type="primary" @click="submitForm('sysModuleAddRuleForm')">保存</el-button>
          <el-button icon="el-icon-caret-left" size="mini" @click="handleClose">返回</el-button>
        </div>
      </div>
      <div class="company-content" v-show='sysModuleShow'>
        ...
        <div class="company-right kl-table">
          ...
          <el-table stripe border :data="appServerData.list" v-if="tableShow" :max-height="maxHeight">
            ...
            <el-table-column prop="matrNo" label="模块路径" min-width="100">
            </el-table-column>
            <el-table-column prop="matrNo" label="模块名称" min-width="100">
            </el-table-column>
            <el-table-column prop="matrName" label="模块类型" min-width="100">
            </el-table-column>
      ...
      <div v-show="sysModuleAddVisible">
        <sysModuleAdd ref="sysModuleAdd"></sysModuleAdd>
      </div>
    </div>
  </div>
</template>
...
<script>
import sysModuleAdd from "./sysModuleAdd"
export default {
  name: "sysModule",
  components: {
    sysModuleAdd
  ...
  methods: {
    ...
    // 添加按钮点击事件
    appAdd(val) {
      this.sysModuleShow = false;
      this.sysModuleAddVisible = true;
      if (val === 'add') {
        this.$refs.sysModuleAdd.editData({
          matrNo: "",
          matrName: "",
          matrMode: "",
          matrNum: "",
        ...
        this.common.chargeObjectEqual(this, this.formData, 'set', 'sysModuleAdd');
      } else {
        ...
        this.$api.fetch({
          ...
          this.$refs.sysModuleAdd.editData(res);
          this.common.chargeObjectEqual(this, res, 'set', 'sysModuleAdd');
        })
      }
    ...
    selectTSubSystem() {
      ...
      //   _this.common.changeTable(_this,'.sysModule .kl-table',[]);
      // })

    },
    closeDialog() {
      this.sysModuleShow = true;
      this.sysModuleAddVisible = false;
      this.crumbsData.titleList.pop();
    },
    // 返回
    handleClose() {

      this.$refs.sysModuleAdd.handleClose();
    },
    //保存按钮
    submitForm(formName) {
      let type = this.crumbsData.titleList[2].title;
      this.$refs.sysModuleAdd.submitForm(formName, type);
    },
